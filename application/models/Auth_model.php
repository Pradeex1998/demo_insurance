<?php
	class Auth_model extends CI_Model{
		...
		public function login($data) {
			...
			if ($validPassword) {
				return $result; // Return user data if password is correct
			} else {
		...
		public function check_password_reset_codeapp($code){

	    	$result = $this->db->get_where('app_user',  array('password_reset_code' => $code ));
	    	if($result->num_rows() > 0){
	    		return true;
	    	}
	    	else{
