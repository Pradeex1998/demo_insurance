...
<div class="row clearfix">
    ...
    function datatable(tableId) {
        var table = $('#'+ tableId).DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('admin/master/state_datatable_json') ?>",
                "type": "POST",
            },
        });
    }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#rtoForm').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: "<?= base_url('admin/master/submit_state') ?>",
                    type: "POST",
                    dataType: 'json',
                    data: $(this).serialize(),
                    success: function(res) {
                        ...
                        if (res && res.status === '1') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Success',
                                text: res.message || 'State saved successfully!',
                                timer: 2000,
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: (res && res.message) ? res.message : 'Failed to save State. Please try again.',
                                timer: 2500,
                                showConfirmButton: false
                            });
                        }
                    ...
                    error: function() {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Failed to save State. Please try again.',
                            timer: 2000,
                            showConfirmButton: false
                        });
                    }
            ...
            $(document).on('click', '.editBtn', function() {
                const id = $(this).data('id');
                $.ajax({
                    url: "<?= base_url('admin/master/get_state_byid') ?>/" + id,
                    type: "GET",
                    ...
                    success: function(response) {
                        if (response.status === "1" && Array.isArray(response.data) && response.data.length > 0) {
                            ...
                            $('#id').val(state.id);
                            $('#name').val(state.name);
                            $('#status').val(state.status).change();
                            $('#rtoModal .modal-title').text('Edit State');
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Invalid State data received.',
                                timer: 2000,
                                showConfirmButton: false
                            });
                        }
                    ...
                    error: function() {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Failed to fetch State details. Please try again.',
                            timer: 2000,
                            showConfirmButton: false
                        });
                    }
            ...
            $('.addBtn').on('click', function() {
                $('#id').val('');
                $('#rtoForm')[0].reset();
                $('#rtoModal .modal-title').text('Add State');
