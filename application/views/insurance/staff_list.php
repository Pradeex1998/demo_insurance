...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="header">
                ...
                <button type="button" data-toggle="modal" data-target="#staffModal" class="btn bg-green waves-effect pull-right addBtn"><i class="material-icons">add</i> Add</button>
            </div>
        </div>
    ...
    function datatable(tableId) {
        var table = $('#'+ tableId).DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('admin/master/staff_datatable_json') ?>",
                "type": "POST",
            },
        });
    }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#staffForm').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: "<?= base_url('admin/master/submit_staff') ?>",
                    type: "POST",
                    dataType: 'json',
                    data: $(this).serialize(),
                    success: function(res) {
                        ...
                        if (res && res.status === '1') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Success',
                                text: res.message || 'Staff saved successfully!',
                                timer: 2000,
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: (res && res.message) ? res.message : 'Failed to save Staff. Please try again.',
                                timer: 2500,
                                showConfirmButton: false
                            });
                        }
            ...
            $(document).on('click', '.editBtn', function() {
                const id = $(this).data('id');
                $.ajax({
                    url: "<?= base_url('admin/master/get_staff_byid') ?>/" + id,
                    type: "GET",
                    ...
                    success: function(response) {
                        if (response.status === "1" && Array.isArray(response.data) && response.data.length > 0) {
                            ...
                            $('#id').val(staff.id);
                            $('#name').val(staff.name);
                            $('#status').val(staff.status).change();
                            $('#staffModal .modal-title').text('Edit Staff');
                            $('#staffForm button[type="submit"]').text('Update');
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Invalid Staff data received.',
                                timer: 2000,
                                showConfirmButton: false
                            });
                        }
            ...
            // Reset modal for Add Staff
            $('.addBtn').on('click', function() {
                $('#id').val('');
                $('#staffForm')[0].reset();
                $('#staffModal .modal-title').text('Add Staff');
                $('#staffForm button[type="submit"]').text('Save');
